...
def gen_greeting(self, night=False):
    if night:
...
def gen_end_msg(self, night=False):
    if night:
...
class ButlerClient:
    def __init__(self, **args):
        self.iclient = icloud_client.iclient(args["iCloud_id"], args["iCloud_pw"])
        self.zipcode = args["zipcode"]
        if "slack_token" in args:
            self.slack_bot = SlackBot.SlackBot(args["slack_token"])

    def send_message(self, message):
        if self.slack_bot is not None:
            self.slack_bot.send_message(config["Slack"]["ChatroomID"], message)

    def get_today_agenda(self):
        agenda = self.iclient.get_today_agenda()
        #agenda = client.get_this_month_schedule()
        return agenda

    def get_tomorrow_agenda(self):
        agenda = self.iclient.get_tomorrow_agenda()
        return agenda

    def gen_agenda_message(self, night=False):
        if night:
            agenda = self.get_tomorrow_agenda()
            mdate = u"내일"
        else:
            agenda = self.get_today_agenda()
            mdate = u"오늘"
        ...
        for item in agenda:
            if first_item:
                ...
                first_item = False
            if item["allDay"]:
                msgs.append(u"%s가 있는 날이고" % item["allDay"])
    ...
    def gen_forecast(self, night=False):
        try:
            if(night):
                mdate = u"내일"
                overall = WeatherUG.getWeeklyFC_Overall(self.zipcode, 1)
                qpf_day = None
            else:
                mdate = u"오늘"
                overall = WeatherUG.getWeeklyFC_Overall(self.zipcode, 0)
                qpf_day = WeatherUG.getHourlyFC_QPF(self.zipcode)
        except Exception, e:
